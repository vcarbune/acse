...
public class Crawler {
	...
	/**
	 *  Reads the stop words from the file given by the stopWordsFile field. 
	 *  The stop words are upperCased and after that added into a hashSet.
	 *   
	 */
	...
	/**
	 * Handles the reading of the terms from the corpus files in 
	 * case the stopWordsFlag is on. It removes 
	 * the stop words from the corpus and counts the position 
	 * of the filtered words into the file after the stop words
	 * were deleted. 
	 * It adds the term plus the document Id and the position of 
	 ...
	 */
	private void processFileStopWordOn(BufferedReader reader, String docID) throws IOException{
		String line;
		int countPos = 0; 
		while ((line = reader.readLine()) != null) {
	...
	private void processLine(BufferedReader reader, String docID) throws IOException{
		String line;
		while ((line = reader.readLine()) != null) {
			StringTokenizer tokens = new StringTokenizer(line);
